...
class CharucoBoardDetection:
    ...
    # expected dtype: np.int32, shape: (n, 1)
    aruco_marker_ids: cvt.MatLike
    # expected dtype: Sequence[np.float32], shape: (n, 4, 2)
    aruco_marker_corners: Sequence[cvt.MatLike]
    # expected dtype: np.int32, shape: (n, 1)
    charuco_ids: Optional[cvt.MatLike] = None
    # expected dtype: np.float32, shape: (n, 4, 2)
    charuco_corners: Optional[cvt.MatLike] = None
    ...
    def has_equal_ids(self, other: Any) -> bool:
        ...
        if not isinstance(other, CharucoBoardDetection):
            return False
        equal_aruco_ids = (
            np.asarray(self.aruco_marker_ids) == np.asarray(other.aruco_marker_ids)
        ).all()
        equal_charuco_ids = (
            np.asarray(self.charuco_ids) == np.asarray(other.charuco_ids)
        ).all()
